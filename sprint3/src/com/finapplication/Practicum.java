	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (command == 1) {
				...
				System.out.println("Всего потрачено: " + expensesManager.getSumTotalExpenses());
			} else if (command == 8) {
				System.out.println("Какую категорию вы хотите удалить?");
				String category = scanner.next();
				expensesManager.removeCategory(category);
			} else if (command == 9) {
				String category = expensesManager.getMaxCategoryExpense();
				System.out.println("В категории " + category + " вы потратили больше всего.");
			} else if (command == 0) {
