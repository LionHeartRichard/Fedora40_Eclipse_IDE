...
class Practicum {
	...
	public static void main(String[] args) {
		...
		if (commandValue.equals(ChatCommand.MAP.name())) {
			System.out.println(MAP_LINK);
		} else if (commandValue.equals(ChatCommand.RECYCLABILITY.name())) {
			handlerByRecyclability(scanner);
		} else if (commandValue.equals(ChatCommand.BONUS.name())) {
			handlerByBonus(scanner);
		} else {
		...
		/*
		 * 
		 * bonus - 1. напечатать сообщение "Введите количество вторсырья, кг:", 2.
		 * добавить ввод значения, 3. рассчитать бонус, умножив вес на коэффициент: если
		 * вес меньше 10 кг, то коэффициент 10, иначе - 15, 4. вывести сообщение
		 * "Количество бонусных баллов: <баллы>."
		 */

	}

	private static void handlerByBonus(Scanner scanner) {
		...
		BigDecimal bigBonus = new BigDecimal(bonus);
		System.out.println("Количество бонусных баллов: " + bigBonus);
	}

	private static void handlerByRecyclability(Scanner scanner) {
		...
		for (RecyclableMaterial material : materials) {
			if (material.getCode() == code) {
				...
				if (material.isRecyclable()) {
					System.out
							.println("Положите его в контейнер с типом «" + containers.get(material.getType()) + "».");
				} else {
	...
	public static void printMenu() {
		...
		System.out.println(
				ChatCommand.RECYCLABILITY + " - проверить по коду на упаковке, подходит ли материал для переработки.");
	...
	public static List<RecyclableMaterial> getMaterials() {
		...
		materials.add(new RecyclableMaterial(23, MaterialType.PAPER, false,
		...
		materials.add(
				new RecyclableMaterial(77, MaterialType.GLASS, false, "стекло, покрытое медью (электроника, часы)"));
		materials.add(new RecyclableMaterial(78, MaterialType.GLASS, false,
				"стекло, покрытое серебром (зеркало, посуда для сервировки)"));
		materials.add(
				new RecyclableMaterial(79, MaterialType.GLASS, false, "позолоченное стекло (посуда для сервировки)"));
